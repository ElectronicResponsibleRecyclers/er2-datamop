...
driveCount=${#nvmes[@]}
  ...
  NEWT_COLORS='root=black,green' whiptail --title "Application Complete" --msgbox "The application has completed.\n\nPress OK to shutdown." 10 60
  if [ $DEBUG -eq 0 ]; then
    shutdown -h now
  fi
  exit 0
...
if [ "$driveCount" -ne $MAX_DRIVES ]; then
  while true; do
    actualDrives=$(whiptail --title "Drive Detection" \
      --inputbox "This machine supports 6 drives but we only detected $driveCount drives. \n\nHow many drives did you insert?" 12 60 3>&1 1>&2 2>&3)

    exitstatus=$?

    # If user pressed Cancel or ESC
    if [ $exitstatus -ne 0 ]; then
      echo
      exit 130
    fi

    # Validate input (must be an integer and equal to driveCount)
    if [[ "$actualDrives" =~ ^[0-9]+$ ]] && [ "$actualDrives" -eq "$driveCount" ]; then
      # ✅ Valid number entered
      break
    else
      # ❌ Invalid input
      NEWT_COLORS='root=white,red' whiptail --title "Invalid Input" \
        --msgbox "Invalid number of drives entered.\n\nWe detected $driveCount drives. Please try again." 10 60
      continue
    fi
  done
...
if [ $missingDrives -ge 1 ]; then
  ...
  while :; do
    read -rp "We are missing $missingDrives drive(s), please scan a serial number: " scannedSerial || {
      echo
      exit 130
    }
    ...
    if ! grep -q "$scannedSerial" <<<"${serials[@]}"; then
      ...
      while :; do
        read -rp "Are you sure this is the serial number? (y/n): " rescanChoice || {
          echo
          exit 130
        }
...
if [ ${#failedDrives[@]} -gt 0 ]; then
  ...
  while [ $failedDriveCount -gt 0 ]; do
    read -rp "We are looking for $failedDriveCount drives, please scan a serial number: " scannedSerial || {
      echo
      exit 130
    }
    if ! grep -q "$scannedSerial" <<<"${serials[@]}"; then
